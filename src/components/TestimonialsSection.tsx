  ...
  <section className="section-padding bg-card">
    <div className="container mx-auto px-4 md:px-8">
      <div className="text-center mb-16">
        <h2 className="text-3xl md:text-4xl text-brand-midnight mb-4 font-serif">Cosa dice chi si è affidato a me</h2>
        <p className="text-brand-smoke italic">Testimonianze di chi ha superato un momento difficile</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {testimonials.map((t) => (
          <div key={t.name} className="bg-brand-ivory p-8 rounded-2xl italic text-brand-smoke">
            <p className="mb-6">"{t.text}"</p>
            <div className="not-italic flex items-center gap-4">
              <div className="w-10 h-10 bg-brand-smoke rounded-full flex items-center justify-center text-primary-foreground font-bold">
                {t.initial}
              </div>
              <span className="font-semibold text-brand-midnight">{t.name}</span>
            </div>
          </div>
        ))}
      </div>
